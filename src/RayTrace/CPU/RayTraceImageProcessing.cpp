...
void RayTraceImageProcessing::rayTraceExercice5() {
...
#pragma omp parallel for schedule(dynamic, 1)
  for (int py = 0; py < image.height(); py++) {
    ...
    for (int px = 0; px < image.width(); px++) {
      ...
      Point o(view.inverse()(
          projection.inverse()(viewport.inverse()(Point(x, y, 0)))));
      // extremite dans l'image
      Point e(view.inverse()(
          projection.inverse()(viewport.inverse()(Point(x, y, 1)))));

      Ray ray(o, e);
      // calculer les intersections
      if (Hit hit = bvh->intersect(ray)) {
        // recuperer le triangle
        const TriangleData &triangle = mesh.triangle(hit.triangle_id);
        // et sa matiere
        const Material &material = mesh.triangle_material(hit.triangle_id);

        // point d'intersection
        Point p = point(hit, ray);
        // normale interpolee du triangle au point d'intersection
        Vector pn = normal(hit, triangle);
        ...
        for (auto s : sources->sources) {
          ...
          for (int i = 0; i < nbRay; ++i) {
            ...
            if (Hit hit1 = bvh->intersect(rayon)) {
              const TriangleData &triangle1 = mesh.triangle(
                  hit1.triangle_id);
              const Material &material1 = mesh.triangle_material(
                  hit1.triangle_id);
              if (material1.emission.power() > 0) {
                ...
                Vector psN = normal(
                    hit1, triangle1);
                if (dot(psN, rayon.d) > 0)
                  psN = -psN;
                float cos_theta_sk = std::max(0.f, dot(psN, normalize(
                    -rayon.d)));
...
// Vu que c'est pas forcément uniforme, pdf peux varier
  ...
  write_image(image, "data/Result/RenderExercice5.png");
  write_image_hdr(image, "data/Result/RenderExercice5.hdr");
}

void RayTraceImageProcessing::rayTraceStartingByColorSource() {
  // recupere les transformations view, projection et viewport pour generer les rayons
  Transform model = Identity();
  Transform view = camera.view();
  Transform projection = camera.projection(image.width(), image.height(), 45);
...
// Vu que c'est pas forcément uniforme, pdf peux varier
  ...
  // enregistrer l'image resultat
  write_image(image, "data/Result/Render.png");
  write_image_hdr(image, "data/Result/Render.hdr");
}