...
struct surface_bezier *load_grille(char *name) {
  FILE *file;
  file = fopen(name, "r+");
  int l, c;
  char tmp;
  ...
  struct surface_bezier *res = new struct surface_bezier;
  res->grille_controle.nb_lignes = l;
  res->grille_controle.nb_colonnes = c;
  res->grille_controle.grille = new Quadruplet *[l];
  for (int i = 0; i < l; i++) {
    res->grille_controle.grille[i] = new Quadruplet[c];
    for (int j = 0; j < c; j++) {
      Quadruplet pt;

      fscanf(file, "%c %lf %lf %lf\n", &tmp, &pt.x, &pt.y, &pt.z);
      pt.h = 1;
      printf("Read point %f %f %f : ", pt.x, pt.y, pt.z);
      res->grille_controle.grille[i][j] = pt;
    }
  ...
  for (int i = 0; i < l; i++) {
    for (int j = 0; j < c; j++) {
      Quadruplet pt = res->grille_controle.grille[i][j];
...
*/
bool getIntersection(struct surface_bezier s, Ray r, float &u, float &v) {
  RayB ray;
  Point pt = r.o + r.d;
  ray.e.x = pt.x;
  ray.e.y = pt.y;
  ...
  // s.grille_controle.grille[0][0];
  if (intersect_surface(&s.grille_controle, &ray, 0.0f, 1.0f, 0.0f, 1.0f, &u, &v))
    return true;
  return false;
}

int main(const int argc, const char **argv) {
  // const char *mesh_filename = "data/rayT/cornell.obj";
  ...
  */
  struct surface_bezier s = *load_grille("data/RayTracingData/bezier.grille");
...
#pragma omp parallel for schedule(dynamic, 1)
  for (int py = 0; py < image.height(); py++) {
    ...
    for (int px = 0; px < image.width(); px++) {
      ...
      Point e = (view.inverse() * projection.inverse() * viewport.inverse())(
          Point(x, y, 1)); // extremite dans l'image
      ...
      if (getIntersection(s, ray, u, v)) {
        ...
        Vector pn = Vector(normQuad.x, normQuad.y,
                           normQuad.z); // normal calculé a partir de la surface
