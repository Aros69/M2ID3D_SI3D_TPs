...
Quadruplet calcul_un_point(Quadruplet *o, int nb_points_controles, float u) {
  Quadruplet* table = new Quadruplet[nb_points_controles];
...
Table_quadruplet calcul_bezier(Quadruplet * points_controle, int nb_points_controles, int nb_points) {
  ...
  res.table = new Quadruplet[nb_points];
  res.nb = nb_points;
...
Grille_quadruplet calcul_surface_bezier(Grille_quadruplet * grille_controle, int nb_points) {
  ...
  resultat.grille = new Quadruplet*[nb_points];
  for (int i = 0; i < nb_points; i++) {
    // ALLOUER(resultat.grille[i], nb_points);
    resultat.grille[i] = new Quadruplet[nb_points];
  }
  ...
  intermediaire.grille = new Quadruplet*[nb_points];
  for (int i = 0; i < nb_points; i++) {
    intermediaire.grille[i]= new Quadruplet[nb_points];
  }
...
void initDerivatives(struct surface_bezier *s)
{
    ...
    s->grille_derive_u.grille = new Quadruplet*[s->grille_controle.nb_lignes];
    ...
    for(int i = 0 ; i < s->grille_derive_u.nb_lignes ; i++)
    {
		...
		s->grille_derive_u.grille[i] = new Quadruplet[s->grille_derive_u.nb_colonnes];
    }

    //Tableau V
    // ALLOUER(s->grille_derive_v.grille,s->grille_controle.nb_lignes);
    s->grille_derive_v.grille = new Quadruplet*[s->grille_controle.nb_lignes];
    ...
    for(int i = 0 ; i < s->grille_derive_v.nb_lignes ; i++)
    {
        ...
        s->grille_derive_v.grille[i] = new Quadruplet[s->grille_derive_v.nb_colonnes];
    }
...
void computeNormal(struct surface_bezier *s)
{
  ...
  // ALLOUER(s->grille_normale.grille, s->grille_controle.nb_lignes );
	s->grille_normale.grille = new Quadruplet*[s->grille_controle.nb_lignes];
    ...
    for(int i = 0 ; i < s->grille_normale.nb_lignes ; i++)
    {
        ...
        s->grille_normale.grille[i] = new Quadruplet[s->grille_normale.nb_colonnes];
...
Quadruplet calcul_un_point2D(Grille_quadruplet ptControle, float u, float v) {
  // Calculer une courbe de bezier sur u
  Quadruplet* curve;
  // ALLOUER(curve, ptControle.nb_lignes);
  curve = new Quadruplet[ptControle.nb_lignes];
  // printf("Affichage courbe bezier contenant le point en u = %f et v = %f\n",u,v);
  for(int i = 0; i < ptControle.nb_lignes ; i++) // Pour chaque
  {
    curve[i] = calcul_un_point(ptControle.grille[i],
                     ptControle.nb_colonnes, u);
...
Quadruplet getNormal(struct surface_bezier *s, double u, double v )
{
  // Calculer un vecteur de derivee u et un vecteur de derivee v
  // et retourne le resultat de leurs cross product
  Quadruplet deriveeU = calcul_un_point2D(s->grille_derive_u,u,v);
  Quadruplet deriveeV = calcul_un_point2D(s->grille_derive_v,u,v);
...
void decoupe_surface(Grille_quadruplet * points_controle, Grille_quadruplet * out_so,
  ...
  left.grille = new Quadruplet*[points_controle->nb_lignes];
	// ALLOUER(right.grille, points_controle->nb_lignes);
  right.grille = new Quadruplet*[points_controle->nb_lignes];
    for (int i = 0; i < points_controle->nb_lignes; i++) {
        // ALLOUER(left.grille[i], (int)ceil(n / 2.) + 1);
        left.grille[i] = new Quadruplet[(int)ceil(n / 2.) + 1];
        // ALLOUER(right.grille[i], (int)ceil(n /2.) + 1);
        right.grille[i] = new Quadruplet[(int)ceil(n /2.) + 1];
    }
  ...
  out_so->grille = new Quadruplet*[out_so->nb_lignes];
	// ALLOUER(out_se->grille, out_se->nb_lignes);
  out_se->grille = new Quadruplet*[out_se->nb_lignes];
	// ALLOUER(out_no->grille, out_no->nb_lignes);
  out_no->grille = new Quadruplet*[out_no->nb_lignes];
	// ALLOUER(out_ne->grille, out_ne->nb_lignes);
  out_ne->grille = new Quadruplet*[out_ne->nb_lignes];
    for (int i = 0; i < out_so->nb_lignes; i++) {
        // ALLOUER(out_so->grille[i], out_so->nb_colonnes);
        out_so->grille[i] = new Quadruplet[out_so->nb_colonnes];
        // ALLOUER(out_se->grille[i], out_se->nb_colonnes);
        out_se->grille[i] = new Quadruplet[out_se->nb_colonnes];
        // ALLOUER(out_no->grille[i], out_no->nb_colonnes);
        out_no->grille[i] = new Quadruplet[out_no->nb_colonnes];
        // ALLOUER(out_ne->grille[i], out_ne->nb_colonnes);
        out_ne->grille[i] = new Quadruplet[out_ne->nb_colonnes];
    }
...
bool intersect_boite(struct RayB * ray, Quadruplet * pMin, Quadruplet * pMax){
...
bool intersect_surface(Grille_quadruplet * grille, struct RayB * ray, 
